 ...
 */
export function buildPagePayload(configSnapshot: Record<string, unknown>, messageKey: string): string {
...
function pageMain(config: Record<string, Record<string, unknown>>, messageKey: string) {
  ...
  type Comment = { idstr: string; text_raw?: string; text?: string; user?: User; comments?: Comment[] };
  ...
  const fetchUserInfo = (function () {
    ...
    const throttled = async function (param: { idstr?: string; screen_name?: string }, config?: { immediate?: boolean }) {
      const cached = returnFromCache(param);
      if (cached) return cached;

      const promise: any = (function () {
        let r: any;
        const p: any = new Promise((res) => (r = res));
        p.resolver = r;
        return p;
      })();
    ...
    const wrapped = async function (param: { idstr?: string; screen_name?: string }, config?: { immediate?: boolean }) {
      const data = await throttled(param, config);
      return JSON.parse(JSON.stringify(data ?? null));
    };

    return {
      id: (id: string, config?: { immediate?: boolean }) =>
  ...
  let _filterConfigCache: { keywords: string[]; authorsSet: Set<string>; removeAd: boolean } | null = null;
  ...
  //#region 监听组件生命周期
  const lifecycleListeners: Record<string, Record<string, Array<{ listener: (instance: any) => any }>>> = {};
  const addLifecycleListener = (lifecycles: string, componentName: string, listener: (instance: any) => any) => {
    const item = { listener };
  ...
  const runLifecycleListeners = (lifecycle: string, instance: any) => {
    ...
    const listenersForComponent = name ? (lifecycleListeners[lifecycle]?.[name] ?? []) : [];
